   ...
   animate() {
    ...
    setupEventListeners() {
        ...
        if (this.isMobile) {
            ...
            tutorial.innerHTML = "Click to enter the gallery. Use W, A, S, D to move, mouse to look, double-click art to focus or open slider, click avatar for help.";
        }
        ...
        document.getElementById("uploadForm")?.addEventListener("change", (e) => this.showImagePreviews(e));
    ...
    update() {
        if (!this.isMobile && this.isLocked && !this.isFocused) {
    ...
    async loadImages(sessionId) {
        try {
            ...
            if (!data.screenshots?.length) {
                ...
                this.imagesToLoad = [...new Set(data.screenshots.map(img => `http://localhost:3000${img}`))];
            }
            console.log("Unique images to load:", this.imagesToLoad);
        ...
        for (let wall of wallConfigs) {
            ...
            for (let { pos, rot } of wallPositions) {
                ...
                try {
                    ...
                    mesh.userData = { filename, hash, baseScale: mesh.scale.clone() };
                    room.add(mesh);
    ...
    openSlider(selectedMesh) {
        if (!this.isMobile && this.isLocked) {
            console.log("Unlocking pointer for slider");
            this.controls.unlock();
            this.isLocked = false; // Sync state immediately
        }
    ...
    closeSlider() {
        ...
        // Do not attempt to lock here; user must click canvas to re-lock
        console.log("Pointer lock not re-engaged automatically; click canvas to resume");
    }
    ...
    updateSliderDisplay() {
        ...
        sliderImage.src = this.sliderImages[this.currentSliderIndex].src;
...
class ThreeJSApp {
    ...
    this.interactiveColumn = null; // New interactive feature
    this.columnLight = null; // Add this to store the column's light
        this.interactionCooldown = 0;
    ...
    createGallery() {
        ...
        const floorMaterial = new THREE.MeshPhysicalMaterial({
            color: 0x111111, // Dark, glossy black
            roughness: 0.1,
            metalness: 0.3,
            reflectivity: 0.9,
            clearcoat: 1.0,
        ...
        // Floor: Reflective circular base
        const floorGeometry = new THREE.CircleGeometry(this.config.roomSize / 2, 64);
        const floor = new THREE.Mesh(floorGeometry, floorMaterial);
        floor.rotation.x = -Math.PI / 2;
        floor.receiveShadow = true;
        room.add(floor);
    
        // Add subtle grid lines to the floor for a futuristic touch
        const gridGeometry = new THREE.RingGeometry(this.config.roomSize / 2 - 0.05, this.config.roomSize / 2, 32);
        for (let i = 1; i <= 5; i++) {
            const gridRing = new THREE.Mesh(gridGeometry, ledMaterial);
            gridRing.rotation.x = -Math.PI / 2;
            gridRing.scale.setScalar(i / 5);
            gridRing.position.y = 0.01; // Slightly above floor
            room.add(gridRing);
        }
    
        // Modern Walls: Floating panels in a semi-circle
        const wallHeight = this.config.wallHeight;
        const radius = this.config.roomSize / 2 - 1;
        const panelCount = 6; // Number of floating panels
        const panelWidth = this.config.roomSize / 4;
        const panelDepth = 0.1;
    
        for (let i = 0; i < panelCount; i++) {
            const angle = (i / panelCount) * Math.PI * 1.5; // Semi-circle arc
            const x = Math.cos(angle) * radius;
            const z = Math.sin(angle) * radius;
    
            const panelGeometry = new THREE.BoxGeometry(panelWidth, wallHeight * 0.8, panelDepth);
            const panel = new THREE.Mesh(panelGeometry, wallMaterial);
            panel.position.set(x, wallHeight * 0.4, z); // Slightly elevated
            panel.rotation.y = -angle + Math.PI / 2; // Face inward
            panel.castShadow = true;
            panel.receiveShadow = true;
            room.add(panel);
    
            // Add a dynamic LED strip along the top edge
            const ledStripGeometry = new THREE.BoxGeometry(panelWidth * 0.9, 0.05, 0.05);
            const ledStrip = new THREE.Mesh(ledStripGeometry, ledMaterial);
            ledStrip.position.set(x, wallHeight * 0.8, z);
            ledStrip.rotation.y = -angle + Math.PI / 2;
            room.add(ledStrip);
    
            // Add a point light for each panel
            const panelLight = new THREE.PointLight(0xffffff, 1.5, this.config.roomSize / 2);
            panelLight.position.set(x, wallHeight * 0.85, z);
            panelLight.castShadow = true;
            room.add(panelLight);
        }
    
        // Glass Wall: Curved transparent backdrop
        const glassPoints = [];
        const glassSegments = 32;
        for (let i = 0; i <= glassSegments; i++) {
            const angle = (i / glassSegments) * Math.PI * 1.5;
            const x = Math.cos(angle) * (radius + 0.5);
            const z = Math.sin(angle) * (radius + 0.5);
            glassPoints.push(new THREE.Vector3(x, 0, z));
        }
        ...
        const glassGeometry = new THREE.ExtrudeGeometry(
            new THREE.Shape([
                ...
                new THREE.Vector2(0.1, 0),
                new THREE.Vector2(0.1, wallHeight * 0.9),
                new THREE.Vector2(-0.1, wallHeight * 0.9)
            ]),
            { depth: 0.1, extrudePath: glassCurve }
        );
        const glassWall = new THREE.Mesh(glassGeometry, glassMaterial);
        glassWall.position.y = 0.01; // Slightly above floor
        glassWall.receiveShadow = true;
        room.add(glassWall);
    
        // Ceiling: Minimal recessed lighting
        const ceilingLightGeometry = new THREE.CircleGeometry(0.5, 32);
        const ceilingLights = [];
        for (let i = -2; i <= 2; i += 2) {
            for (let j = -2; j <= 2; j += 2) {
                const lightPanel = new THREE.Mesh(ceilingLightGeometry, ledMaterial);
                lightPanel.position.set(i * 2, wallHeight, j * 2);
                lightPanel.rotation.x = Math.PI / 2;
                room.add(lightPanel);
    
                const ceilingLight = new THREE.PointLight(0xffffff, 0.8, this.config.roomSize);
                ceilingLight.position.set(i * 2, wallHeight - 0.1, j * 2);
        ...
        // Floating Art Platforms
        const platformCount = 4;
        const platformRadius = radius * 0.6;
        for (let i = 0; i < platformCount; i++) {
            ...
            platform.position.set(x, 0.5, z); // Hovering above floor
            platform.castShadow = true;
            platform.receiveShadow = true;
            room.add(platform);
    
            // Add a subtle glow effect
            const glowGeometry = new THREE.RingGeometry(1.4, 1.5, 32);
            const glowRing = new THREE.Mesh(glowGeometry, ledMaterial);
            glowRing.position.set(x, 0.51, z);
            glowRing.rotation.x = -Math.PI / 2;
            room.add(glowRing);
        }
    
        // Central Interactive Feature: Rotating Light Column
        const columnGeometry = new THREE.CylinderGeometry(0.3, 0.3, wallHeight * 0.9, 32);
        const column = new THREE.Mesh(columnGeometry, glassMaterial);
        column.position.set(0, wallHeight * 0.45, 0);
        column.castShadow = true;
        room.add(column);
    
        const columnLight = new THREE.PointLight(0x00aaff, 2, this.config.roomSize);
        columnLight.position.set(0, wallHeight * 0.9, 0);
        columnLight.castShadow = true;
        room.add(columnLight);
    
        // Store references
        this.interactiveColumn = column;
        this.columnLight = columnLight; // Store the light reference
   ...
   animate() {
    ...
    updateObjectAnimations() {
        if (this.isAnimatingObjects) {
            ...
            // Removed lightWall reference as it's not part of the modern design
        }
    }
    updateImageEffects() {
        this.images.forEach((img, index) => {
    ...
    update() {
        ...
        // Animate the interactive column
        if (this.interactiveColumn) {
            this.interactiveColumn.rotation.y += 0.02;
            const dist = this.camera.position.distanceTo(this.interactiveColumn.position);
            const intensity = Math.max(0.5, Math.min(2, 5 / dist));
            this.interactiveColumn.material.opacity = 0.5 + Math.sin(this.time) * 0.2;
            if (this.columnLight) { // Check if the light exists
                this.columnLight.intensity = intensity;
            }
        }
    ...
    const room = this.rooms[0];
    const platforms = room.children.filter(child => child.geometry instanceof THREE.CylinderGeometry && child.position.y === 0.5);
    const platformCount = platforms.length;

    for (let i = 0; i < platformCount && imageIndex < totalImages; i++) {
        ...
        try {
            ...
            const geometry = new THREE.PlaneGeometry(adjustedWidth, this.config.displayHeight);
            const mesh = new THREE.Mesh(geometry, material);
            mesh.position.set(platform.position.x, 1.5, platform.position.z); // Above platform
            mesh.castShadow = true;
            mesh.receiveShadow = true;
            mesh.userData = {
                filename,
            ...
            const frame = new THREE.Mesh(frameGeometry, frameMaterial);
            frame.position.copy(mesh.position);
            frame.position.z -= 0.01; // Slightly behind image
            frame.castShadow = true;
            frame.receiveShadow = true;
            room.add(frame);

            const spotlight = new THREE.SpotLight(0xffffff, 2.0, this.config.roomSize, Math.PI / 6, 0.7);
            spotlight.position.set(platform.position.x, this.config.wallHeight - 0.5, platform.position.z);
            spotlight.target = mesh;
            spotlight.castShadow = true;
            spotlight.shadow.mapSize.width = 512;
            spotlight.shadow.mapSize.height = 512;
